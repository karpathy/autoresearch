"""
Autoresearch training script — 10-digit addition task.
Trains the smallest transformer that achieves >=99% exact-match accuracy.
val_bpb = 1 - accuracy  (lower is better; target < 0.01)
Usage: uv run train.py   (run from the addition/ directory)
"""
...
class GPTConfig:
    ...
    n_kv_head:    int = 2
    n_embd:       int = 128
    window_pattern: str = "L"   # full attention — no sliding window for short seqs
...
# Optimization
TOTAL_BATCH_SIZE = 2**15  # 32768 tokens per optimizer step (~512 examples)
DEVICE_BATCH_SIZE = 512   # per-device micro-batch; must satisfy TOTAL/DEVICE/SEQ_LEN ∈ ℤ
EMBEDDING_LR     = 0.6
...
while True:
    ...
    print(
        ...
        f"mfu: {mfu:.1f}% | remaining: {remaining:.0f}s    ",
        end="", flush=True,
    )
