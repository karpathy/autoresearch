...
### Standard CMA-ES:
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/optimize-cmaes.py --n_trials 500
```

### With restart strategy for multimodal problems:
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/optimize-cmaes.py --restart ipop --n_trials 1000
```

### BIPOP variant:
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/optimize-cmaes.py --restart bipop --n_trials 2000
```
