...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/optimize-hybrid.py --strategy auto --n_trials 200
```

### Sequential multi-phase:
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/optimize-hybrid.py --strategy sequential --n_trials 500
```

### Force CatCMA (best for mixed variables):
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/optimize-hybrid.py --strategy catcma --n_trials 300
```
