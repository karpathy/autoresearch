...
def train_tokenizer():
    """Train BPE tokenizer using rustbpe, save as JSON (safe, no pickle)."""
    ...
    # Save tokenizer components as JSON (safe, no pickle)
    tokenizer_data = {
        "name": "rustbpe",
        "pat_str": pattern,
        "mergeable_ranks": {
            base64.b64encode(k).decode("ascii"): v for k, v in mergeable_ranks.items()
        },
        "special_tokens": special_tokens,
    }
...
class Tokenizer:
    ...
    def from_directory(cls, tokenizer_dir=TOKENIZER_DIR):
        ...
        if os.path.exists(json_path):
            ...
            print(
                "WARNING: Loading tokenizer from pickle (unsafe). "
                "Re-run 'python prepare.py' to regenerate as safe JSON."
            )
            import pickle
            ...
            raise FileNotFoundError(
                f"No tokenizer found in {tokenizer_dir}. Run 'python prepare.py' first."
            )
        return cls(enc)
