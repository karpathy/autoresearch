...
def require_cuda_environment(context):
    if not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "Install a CUDA-enabled PyTorch build and run on a machine with an available NVIDIA GPU. "
            "If you are on CPU, Apple Silicon, or another unsupported platform, use a platform-specific fork instead; see README.md for notable forks."
        )
...
# ---------------------------------------------------------------------------
# Setup + training entrypoint
# ---------------------------------------------------------------------------

def main():
    global fa3

    cap = require_cuda_environment("train.py")
