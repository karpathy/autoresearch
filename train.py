...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        _, T, _ = x.size()
        q = self.c_q(x).view(-1, T, self.n_head, self.head_dim)
        k = self.c_k(x).view(-1, T, self.n_kv_head, self.head_dim)
        v = self.c_v(x).view(-1, T, self.n_kv_head, self.head_dim)

        # Value residual (ResFormer): mix in value embedding with input-dependent gate per head
        if ve is not None:
            ve = ve.view(-1, T, self.n_kv_head, self.head_dim)
        ...
        y = y.contiguous().view(-1, T, self.n_embd)
        y = self.c_proj(y)
        return y
...
while True:
    ...
    for micro_step in range(grad_accum_steps):
        if ddp:
            model._orig_mod.require_backward_grad_sync = (
